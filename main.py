...
def fetch_new_shorts(channel):
    ...
    for index, short_id in enumerate(shorts_id):
        ...
        old_published_at = latest_short.get("published_at")
        if old_published_at is None or (published_at and published_at > latest_short.get("published_at")):
...
def fetch_new_videos(channel):
    ...
    for index, video_id in enumerate(videos_id):
        ...
        old_published_at = latest_video.get("published_at")
        if old_published_at is None or (published_at and published_at > latest_video.get("published_at")):
...
def fetch_new_streams(channel):
    ...
    for index, stream_id in enumerate(streams_id):
        ...
        old_published_at = latest_stream.get("published_at")
        if old_published_at is None or (published_at and published_at > latest_stream.get("published_at")):
